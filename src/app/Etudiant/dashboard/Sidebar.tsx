...
import { LayoutDashboard, FileText, CreditCard, MessageCircle, LogOut } from "lucide-react";
...
export default function Sidebar({ darkMode }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { name: "Formations", icon: <FileText size={20} color={iconColor} /> },
    { name: "Paiements", icon: <CreditCard size={20} color={iconColor} /> },
    { name: "Messages", icon: <MessageCircle size={20} color={iconColor} /> },
  ];
